...
export type AntiStyleConfrontationBooleans = {
    ...
    /** 抗「总分总」式开篇：禁止首句概括全段/全章要点 */
    antiTotalPartTotalStructure: boolean;
};
...
- **对白与旁白同理**：叙述与人物台词均避免为了「书面感」而堆砌「不是……而是……」类拐弯说法；该肯定就肯定，该否定就否定。`
        : '';
    const antiTotalPartTotalBlock = opts.antiTotalPartTotalStructure
        ? `

**抗「总分总」结构（严禁开篇一句概括全段或全章）**：
- **严格禁止**在正文**第一句**（或紧接开篇的前一两句）用概括性、预告性语句**总结本段或本章将要发生的全部内容**（如「这一夜注定……」「接下来会发生……」「一切要从……说起」「本段将展现……」类总起）。
- **禁止**为凑「总—分—总」而在开头先抛全段「论点式概括」再展开；应**直接进入具体场景、动作、对话或感官细节**，让信息从情节中自然流出。
- **不误伤**：单句环境/时间切入（如「凌晨三点，雨还没停。」）若未概括剧情走向或后续要点，不视为违规。`
        : '';
    return (
        basePrompt +
        antiLovecraftBlock +
...
export function antiStyleFlagsFromRequestBody(body: Record<string, unknown> | null | undefined): AntiStyleConfrontationBooleans {
    ...
    return {
        ...
        antiTotalPartTotalStructure: b.anti_total_part_total_structure !== false,
    };
}
...
4. **滥用网络安全/加密通信术语**：仅当正文出现「加密通讯」「加密标识」「VPN」「端到端加密」「安全信道」「加密链路」「加密频道」「加密频段」等**网络安全/加密通信技术术语本身**时，才判为不通过。审稿时严禁将该条外扩到其他“现代感”词汇（如商业词、网络流行语、日常口语等）进行误判；这些不属于本条检测范围。所有**网络安全/加密通信技术术语本身**意象，必须删除或改写，没有妥协余地。
5. **擅自引入「通用语」类设定（严格执行）**：
...
21. **「总分总」式开篇与首句全文总结（从严判定）**：若正文**开篇第一句或前两句**以概括、预告、论点式语句**打包本段或本章核心情节、将要发生的全部要点或结局走向**（替读者预先知道「将要发生什么」、把后文剧透在前两句说完），判为不通过。典型包括：剧情预告式总起、「这一夜/这一次/接下来」引出的**结论式概括**、开头即点明冲突结果或主题升华句、开篇一句像段意概括或章纲摘要。**要求**改为从具体时空、动作、对话或细节切入，让情节展开后再呈现信息。**不误伤**：仅交代时间/地点/氛围的单句切入，且**未概括**后续剧情或要点，不判不通过。
...
4. **提升文风灵动感（新增，防止“一律保守严肃化”）**：
   ...
   - **判定条件（避免误伤）**：若当前场景是谈判、战场、葬礼，则不适用此规则。
   - 给建议时必须可执行：指出哪些段落可放松语体，并给出“如何从严肃腔改成口语化（可略带轻佻）”的改写方向（如替换过度书面句式、补入更自然的口语化表达、保留信息同时降低宣讲感）。
5. **润色阶段设定边界（新增硬约束）**：
...
7. **段落开头策略分析**：
   ...
   - 但以上判据的前提是，本章节已经覆盖到了章节纲要开头的部分，如果本章节未从纲要的开头部分开始写，而是从中间部分开始写，甚至回溯到前文重写前面的内容，那么开头策略分析不适用，你需要提醒writer理解有误，开头应该是什么，必须完整地从段纲开头部分写起
   ...
   - **首句全文总结（与1号审稿员第21条同一标准）**：若**开篇第一句或前两句**以概括、预告式语句总结本段或本章将要发生的全部内容、预先打包剧情结论或段意式总起，必须在建议中要求**删除或改写该总起**，改为从具体场景、动作、对话或感官细节切入；判定口径与 1 号第 21 条一致，并在 FAIL 时指出原句与改写方向。
...
12. **逻辑问题检查**
   ...
   - 检查对话是否逻辑不通，比如A说了一件事，结果B回应的是不一样的事情，这种情况下，你需要提出重建对话的意见。
