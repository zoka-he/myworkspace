...
5. **擅自引入「通用语」类设定（严格执行，有疑即判不通过）**：
   - **默认禁止**：除非在【章节背景】或【相关设定】中**明确写出**存在某种通用/通行语言，否则正文中不得出现任何「存在一种大家都会说的语言」的表述或暗示。
   - **必检表述（含变体与近义）**：凡出现以下任一类即判不通过——「通用语」「大陆通用语」「世界通用语」「各族通用语」「全境通用语」「通行语」「共同语」「标准语」（指全境唯一时）、「用一种语言」「同一种语言」「双方都懂的语言」「他们都能听懂的话」「无需翻译就能交流」「用大家都懂的话」等；以及含混表述如「以某种通用语言」「用通行语」「用共同语」。
   - **审稿要求**：对与「语言/交流方式」相关的每一处表述保持敏感。凡**暗示**或**明示**存在一种未经前文或设定**明确交代**的全境/多族/多方通用语言或「人人都懂」的交流方式，一律判不通过；无法在给定【章节背景】【本章待写内容】及【相关设定】中找到明确依据的，视为违规。
