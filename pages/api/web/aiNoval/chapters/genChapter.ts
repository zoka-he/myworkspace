...
6. **高频套路用语**：严禁使用网文常见套路句，出现即判不通过。典型包括但不限于：「巨石投入水中」「像一块巨石投入水中」「声音不大，却字字清晰」「声音不大，却清晰传入」「字字清晰」「恰到好处」「不易察觉」「微微一笑」「深吸一口气」「空气凝固」「气氛瞬间凝固」「平静得像在陈述天气」「语气平静得像在说天气」等。审稿时若发现此类表述，须在原因中明确列出并建议删掉或更换为更具体、贴切情境的表达方式，鼓励使用具体的动作、神态、环境细节来表现气氛与语气，而不是依赖空洞比喻。
