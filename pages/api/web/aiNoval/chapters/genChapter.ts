...
6. 使用2010年后中文网络化的表达，避免出现生硬的英式/翻译腔；不要写出「他开口说道」「他缓缓开口道」「他顿了顿道」「她沉默了片刻才说道」这类直译自英文叙述套路的句子，优先使用自然的中文说法。
...
6. **高频套路用语**：严禁使用网文常见套路句，出现即判不通过。典型包括但不限于：「巨石投入水中」「像一块巨石投入水中」「声音不大，却字字清晰」「声音不大，却清晰传入」「声音清晰，却……」「字字清晰」「恰到好处」「不易察觉」「微微一笑」「深吸一口气」「空气凝固」「气氛瞬间凝固」「平静得像在陈述天气」「语气平静得像在说天气」等。审稿时若发现此类表述，须在原因中明确列出并建议删掉或更换为更具体、贴切情境的表达方式，鼓励使用具体的动作、神态、环境细节来表现气氛与语气，而不是依赖空洞比喻。
...
11. **叙述者解释剧情/动机/因果**：正文中不得用叙述者口吻或旁白直接解释剧情、动机、因果或「为什么会这样」。若出现「原来……」「这意味着……」「事实上……」「因为……所以……」「他这样做的原因是……」等解释性句子，或替读者总结本段/前文发生了什么事，判为不通过；情节应通过行动、对话和细节展现（展示而非告知），不得用说明性语句替代。
