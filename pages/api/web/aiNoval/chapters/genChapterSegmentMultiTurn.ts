...
3. **概括式剧透**：用概括性语句代替具体情节（例如「后来他们经历了种种终于……」），导致本该在本段呈现的情节被一笔带过或剧透。**另（「总分总」式开篇）**：若本段正文**第一句或前两句**以概括、预告式语句总结本段将要写的全部要点、预先打包剧情结论或段意式总起，视同概括式剧透，判为不通过；须改为从具体动作、对话或场景细节直接切入。结尾同理，在章节到达结尾时，不应再强行推进内容，或改变章节叙事的结果。**不误伤**：仅时间/地点/氛围的单句切入且未概括后续剧情或提纲要点，不判不通过。
...
- **首句全文总结（与1号分段审稿第3条「总分总」补充同一标准）**：若开篇第一句或前两句以概括、预告式语句总结本段将要发生的全部要点或预先打包结论，必须要求删除该总起并改为具体切入；判定与 1 号一致。
- **章节尾部防升华**：任何抒情/擅自推进状态的章节末尾都必须禁止，比如让扔在进行中的会议突然结束，让没有动地载具动起来，角色突然抒发明显不同于剧情的情感，都视为模板写作，应提出改写回正方案，回正目标是保持悬而未决的状态。
...
export interface GenChapterSegmentMultiTurnInput {
  ...
  /** 抗「总分总」式开篇：禁止首句概括全段要点，默认开启 */
  anti_total_part_total_structure?: boolean;
  /** 是否启用审稿员（多轮文风纠正），默认关闭 */
...
**旁白禁止为求安全而抹平剧情**：叙述层不要用「双方交换了意见」「进行了沟通」「气氛有些紧张」「情绪有些波动」等空泛、去冲突的**安全表达**顶替本段应有的对峙、失败、愤怒或张力；提纲或对白里已有的锋芒，旁白不得用公文腔、新闻腔压平。`;
  return appendAntiStyleConfrontationBlocks(basePrompt + segmentExtra, {
    antiLovecraftStyle,
    antiSweetCeoStyle,
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<Data>) {
  ...
  const {
    ...
    anti_total_part_total_structure = true,
