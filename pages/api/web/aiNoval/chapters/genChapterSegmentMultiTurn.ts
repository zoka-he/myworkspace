...
16. **大幅度改变原始剧情**：待审稿内容必须与【本段提纲】及【前序章节摘要】所确立的剧情方向一致，不得擅自改写关键情节、人物动机、事件结果或世界观设定。若本段正文**明显**偏离提纲要点（如提纲要求写 A 发生、正文却写成 B 发生或 A 未发生）、或与前文已确立的情节/人物状态严重矛盾、或凭空加入提纲与设定中不存在的重要事件或转折，判为不通过，要求重写。**不误伤**：合理的细节扩展、对话与动作的丰富化、以及在不违背提纲前提下的微调不判不通过；仅当出现**大幅度**偏离或颠覆既定剧情时才判不通过。审稿时须在原因中具体说明偏离了哪些要点或与前文/设定何处矛盾。
17. **语境要求阴阳怪气/拌嘴却写得平淡**：当【本段提纲】、【前序章节摘要】或世界观设定中，明确或隐含要求「阴阳怪气」「嘴上不饶人」「互相挤兑」「拌嘴/吵架」「针锋相对」「暗中较劲」「火药味」等风格（例如：本段是争吵、互相损、冷嘲热讽、暗中斗嘴的场景），而【待审稿内容】中的对话却整体语气温和、礼貌、缺乏情绪张力，只是礼貌性交换信息或轻描淡写几句、几乎没有刺痛感和情绪波动，则判为不通过。审稿时需要：1）在原因中明确指出「提纲要求的是带火药味/阴阳怪气/互相挤兑的对话，但正文语气过于平，缺乏情绪冲突」；2）**必须修改**：增加具体的反讽、抬杠、夹枪带棒、冷嘲热讽等带情绪和立场的台词，以及相应的小动作、神态与节奏变化，让对话真正体现出拌嘴/吵架的味道，禁止保留公事公办式互通信息。
18. **段纲情感基调被抹平（必须制止）**：模型常把带情绪、带张力的段纲写成「温吞、客观、信息量足但无情绪」的中性叙述，等于**抹平**作者在本段要求的情感走向。**你必须主动识别【本段提纲】中的情感基调**（如：悲怆、亢奋、压抑、温馨、紧张、戏谑、愤怒、惶恐、释然、暧昧、冷峻、荒诞等——凡提纲用词、场景性质能推断出明显情绪色彩的，均算带基调）。若提纲明显带某种基调，而正文整体读起来**情绪平淡、像在写说明书或新闻摘要**、节奏与用词都偏「安全、抹平」，与提纲应有的张力或冷暖明显不符，判为不通过。**必须修改**中须写明：提纲要求的基调是何种、正文何处被抹平、**必须补入或改写**为何种情绪载体（对白语气、叙述节奏、意象、动作细节等）。**不误伤**：提纲本身未体现任何情感倾向、仅交代情节要点时，不因「略平」判不通过；仅当提纲**明显**带基调而正文**明显**抹平时判不通过。本条与第 17 条可同时适用；第 17 条专指拌嘴/火药味类，本条覆盖一切段纲情感被弱化的情形。
19. **争吵场景堆砌反问（灌水，必须删）**：写争吵、拌嘴、质问、对峙时，模型常塞入大量**反问句**（如「你难道不知道……？」「这不是明摆着吗？」「你以为我是谁？」「你觉得呢？」等，或者简单重复前一人说的话）撑篇幅，信息量为零或极低，属**灌水**；且反问在后续润色时**极易漏删**。当本段为争吵/冲突类对话时，你必须**逐句评估**每一处反问：**是否非它不可**？若去掉后意思不变或反而更利落，即属可删。**默认立场：大部分反问必须删除**，改为直陈、反驳、具体指控或带信息的短句。若同一段争吵里反问连续出现或占比过高，判为不通过；**必须删除**中须逐条列出须删的反问及删后应用何种陈述替代（或写「整段反问仅保留 X 处，其余必须删除并改为……」）。**不误伤**：反问本身承载关键情节信息（如揭穿谎言的唯一一句）或人物标志性说话方式时，可保留；仅针对明显凑节奏、凑字数的反问堆叠。
20. **旁白安全表达抹平剧情（与第 18 条分工）**：第 18 条看**整段**相对段纲是否被抹平；本条专看**旁白叙述层**是否为了「安全、不得罪人、不站队」而刻意用**中性、空泛、去冲突**的措辞，把本该有锋芒、张力或明确态度的情节**写成温吞摘要**。典型表现：该写对峙却写成「双方交换了意见」「气氛一度有些紧张」；该写失败/羞辱却写成「事情并未完全按预期发展」；该写愤怒/恐惧却用「情绪有些波动」带过；用「进行了沟通」「达成了某种共识」等**公文腔、新闻腔**消解戏剧冲突；用「似乎」「某种程度上」「不好说」等**模糊语**逃避写出真实局面。若【本段提纲】或对话本身已呈现冲突、转折或强烈情绪，而旁白仍持续使用上述**安全壳**表述、导致读者读不出应有的尖锐或沉重，判为不通过。**必须修改**中须指出哪些旁白句属于安全抹平、**必须改写为**带具体动作、感官或立场的叙述（可保留客观视角，但不得用空泛中性句顶替应有张力）。**不误伤**：纯说明性设定展开、纪实风格段落、或提纲确为平淡过渡时，不因中性叙述判不通过；仅当**情节/对白已有张力而旁白明显在压平**时判不通过。与第 18 条可同时触发。

{{NON_FINAL_SEGMENT_RULE}}
