...
export default class InteractService extends MysqlService {
    constructor() {
        super('t_interact');
    }
    ...
    queryWithTaskName(conditions, page = 1, limit = 20) {
        ...
        let sql, values = [], baseSql = `select * from t_interact ti left join (select task_name, ID as tid from t_task tt) as tt on ti.task_id=tt.tid`;
