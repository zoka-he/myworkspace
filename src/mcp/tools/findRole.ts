...
export class FindRoleTool extends BaseMCPTool {
  readonly definition: MCPToolDefinition = {
    name: 'find_role',
    description: '在指定世界观下按关键词检索角色，返回相似度≥threshold的结果。',
    inputSchema: {
      type: 'object',
      properties: {
        worldview_id: {
  ...
  async execute(args: {
    ...
    try {
      ...
      return {
        content: [{ type: 'text' as const, text: `find_role 执行失败: ${e?.message || String(e)}` }],
        isError: true,
      };
    }
  }
